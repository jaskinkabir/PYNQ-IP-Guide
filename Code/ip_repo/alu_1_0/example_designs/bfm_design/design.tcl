proc create_ipi_design { offsetfile design_name } {
	...
	set alu_0 [ create_bd_cell -type ip -vlnv user.org:user:alu:1.0 alu_0]
...
set_property -name {xsim.simulate.runtime} -value {100ms} -objects [get_filesets sim_1]
	...
	set offset_file "${bd_path}/alu_tb_include.svh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef alu_tb_include_vh_"
	puts $fp "`define alu_tb_include_vh_\n"
...
set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores user.org:user:alu:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/alu_tb.sv
...
remove_files -quiet -fileset sim_1 alu_tb_include.vh
